...
type DatadogPodAutoscalerPolicy struct {
	// DryRun is a global setting to entirely skip actions on the target resource.
...
type DatadogPodAutoscalerStatus struct {
	// RecommendationsVersion holds the last received version of the recommendation
	// +optional
	RecommendationsVersion *uint64 `json:"recommendationsVersion,omitempty"`

	// UpdateTime is the timestamp at which the last recommendation data was received
	// +optional
